...
db.on('ready', () => {
  ...
  db.tables.forEach(table => {
    ...
    // Updating hook - updates the updated_at timestamp
  ...
  // Special hooks for sync-critical tables
  const syncCriticalTables = [
    'customers', 'vendors', 'suppliers', 'labour',
    'jobs', 'invoices', 'purchases', 'vouchers'
  ];
  
  syncCriticalTables.forEach(tableName => {
    const table = db.table(tableName);
    
    // Add to sync queue on any modification
...
const queueSyncOperation = async (tableName, operation, data) => {
  try {
    // Check if database is open and syncQueue table exists
    if (!db.isOpen()) {
      console.warn('[DEXIE:SYNC_QUEUE] Database not open, skipping queue operation');
      return;
    }
    
    if (!db.syncQueue) {
      console.warn('[DEXIE:SYNC_QUEUE] syncQueue table not available');
      return;
    }
    ...
    // Silently handle NotFoundError to avoid console spam
    if (error.name !== 'NotFoundError') {
      console.error('[DEXIE:SYNC_QUEUE] Failed to queue operation:', error);
    }
  }
};
