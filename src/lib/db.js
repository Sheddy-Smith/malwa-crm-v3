...
export const dbTransaction = async (storeNames, mode, callback) => {
  ...
  return await database.transaction(dexieMode, tables, async () => {
    // Create a transaction-like object for backward compatibility
    const txn = {
      objectStore: (name) => database.table(name)
    };
