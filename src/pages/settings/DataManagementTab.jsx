...
const DataManagementTab = () => {
  ...
  const loadSystemStatus = async () => {
    try {
      ...
      // Get sync status
      const status = await comprehensiveSyncManager.getSyncStatus();
      setSyncStatus(status);
      
      // Get system health
      const health = await comprehensiveSyncManager.getSystemHealth();
  ...
  const initializeSystem = async () => {
    try {
      ...
      const result = await comprehensiveSyncManager.initializeSync();
  ...
  const performFullSync = async () => {
    try {
      ...
      const result = await comprehensiveSyncManager.performFullSync();
  ...
  const exportAllData = async () => {
    try {
      ...
      const result = await comprehensiveSyncManager.exportAllData();
            ...
            onClick={async () => {
              try {
                ...
                if (window.electron?.fs) {
                  ...
                  const testResult = await window.electron.fs.ensureDir(testPath);
