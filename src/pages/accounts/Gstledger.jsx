...
const Gstledger = () => {
  ...
  const loadOpeningBalance = async () => {
    try {
      ...
      if (gstSetting) {
        setOpeningBalance(parseFloat(gstSetting.value) || 0);
      }
    } catch (error) {
    ...
    try {
      ...
      if (gstSetting) {
        await dbOperations.update('settings', gstSetting.id, {
          ...gstSetting,
          value: tempOpeningBalance.toString()
        });
      } else {
    ...
    try {
      ...
      const purchaseEntries = (purchases || [])
        ...
        .map(p => {
          ...
          return {
            ...
            igst,
            totalGST,
            gstType: gstType === 'igst' ? 'IGST' : 'CGST+SGST',
      ...
      const sellEntries = (invoices || [])
        ...
        .map(inv => {
          ...
          const gstType = inv.gst_type || 'igst';
  ...
  const handlePrint = () => {
    try {
      const success = printContent('gst-ledger-print-view');
      if (!success) {
  ...
  return (
    <div className="p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 dark:bg-gray-800">
                <tr>
                  <th className="p-3 text-left font-semibold" style={{ width: '10%' }}>Date</th>
                  <th className="p-3 text-left font-semibold" style={{ width: '8%' }}>Type</th>
                  <th className="p-3 text-left font-semibold" style={{ width: '18%' }}>Party Name</th>
                  <th className="p-3 text-left font-semibold" style={{ width: '10%' }}>Invoice No</th>
                  <th className="p-3 text-right font-semibold" style={{ width: '10%' }}>Taxable Amount</th>
                  <th className="p-3 text-right font-semibold" style={{ width: '8%' }}>CGST</th>
                  <th className="p-3 text-right font-semibold" style={{ width: '8%' }}>SGST</th>
                  <th className="p-3 text-right font-semibold" style={{ width: '8%' }}>IGST</th>
                  <th className="p-3 text-right font-semibold" style={{ width: '10%' }}>Total GST</th>
                  <th className="p-3 text-center font-semibold" style={{ width: '15%' }}>GST Number</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry, index) => (
                  <tr key={index} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                    ...
                    <td className="p-3 text-right">₹{(entry.taxableAmount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                    <td className="p-3 text-right">₹{(entry.cgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                    <td className="p-3 text-right">₹{(entry.sgst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                    <td className="p-3 text-right">₹{(entry.igst || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                    <td className="p-3 text-right font-semibold">₹{(entry.totalGST || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
      ...
      <div style={{ display: 'none' }}>
        <div id="gst-ledger-print-view">
          <PrintableView
            title="GST Ledger Report"
            subtitle={`Period: ${formatDate(dateRange.from)} to ${formatDate(dateRange.to)}`}
            headerInfo={[
              { label: 'Opening Balance', value: formatCurrency(openingBalance) },
              { label: 'Input GST', value: formatCurrency(summary.inputGST) },
              { label: 'Output GST', value: formatCurrency(summary.outputGST) },
          ...
          >
            <div className="print-summary-grid">
              <div className="print-summary-item">
                <div className="print-summary-label">Opening Balance</div>
                <div className="print-summary-value">{formatCurrency(openingBalance)}</div>
              </div>
              <div className="print-summary-item">
                <div className="print-summary-label">Input GST</div>
                <div className="print-summary-value">{formatCurrency(summary.inputGST)}</div>
              </div>
              <div className="print-summary-item">
                <div className="print-summary-label">Output GST</div>
                <div className="print-summary-value">{formatCurrency(summary.outputGST)}</div>
              </div>
              <div className="print-summary-item">
                <div className="print-summary-label">Net GST</div>
                <div className="print-summary-value" style={{ color: summary.netGST >= 0 ? '#212529' : '#dc3545' }}>
                  {formatCurrency(summary.netGST)}
                </div>
              </div>
              <div className="print-summary-item">
                <div className="print-summary-label">Closing Balance</div>
                <div className="print-summary-value" style={{ color: closingBalance >= 0 ? '#212529' : '#dc3545' }}>
              ...
              <tbody>
                {entries.map((entry, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ textAlign: 'right' }}>{formatCurrency(entry.taxableAmount || 0)}</td>
                    <td style={{ textAlign: 'right' }}>{formatCurrency(entry.cgst || 0)}</td>
                    <td style={{ textAlign: 'right' }}>{formatCurrency(entry.sgst || 0)}</td>
                    <td style={{ textAlign: 'right' }}>{formatCurrency(entry.igst || 0)}</td>
                    <td style={{ textAlign: 'right' }}><strong>{formatCurrency(entry.totalGST || 0)}</strong></td>
              ...
              <tfoot>
                <tr>
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <strong>{formatCurrency(entries.reduce((sum, e) => sum + (e.taxableAmount || 0), 0))}</strong>
                  </td>
                  <td style={{ textAlign: 'right' }}>
                    <strong>{formatCurrency(entries.reduce((sum, e) => sum + (e.cgst || 0), 0))}</strong>
                  </td>
                  <td style={{ textAlign: 'right' }}>
                    <strong>{formatCurrency(entries.reduce((sum, e) => sum + (e.sgst || 0), 0))}</strong>
                  </td>
                  <td style={{ textAlign: 'right' }}>
                    <strong>{formatCurrency(entries.reduce((sum, e) => sum + (e.igst || 0), 0))}</strong>
                  </td>
                  <td style={{ textAlign: 'right' }}>
                    <strong>{formatCurrency(entries.reduce((sum, e) => sum + (e.totalGST || 0), 0))}</strong>
                  </td>
                  <td></td>
                </tr>
              </tfoot>
