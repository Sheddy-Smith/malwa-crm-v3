...
const JobHistoryTab = ({ customer }) => {
  ...
  const fetchJobs = async () => {
    try {
      setLoading(true);
      let data = await dbOperations.getAll('customer_jobs') || [];
      
      // Filter by customer
      data = data.filter(job => job.customer_id === customer.id);
      
      // Sort by date descending
      data.sort((a, b) => String(b.job_date || '').localeCompare(String(a.job_date || '')));
