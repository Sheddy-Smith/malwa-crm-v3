...
const SalesHistoryTab = () => {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      let data = await dbOperations.getAll('invoices') || [];
      
      // Filter by customer
      data = data.filter(inv => inv.customer_id === selectedCustomerId);
      
      // Apply filters
      if (startDate) data = data.filter(inv => inv.invoice_date >= startDate);
      if (endDate) data = data.filter(inv => inv.invoice_date <= endDate);
      if (statusFilter !== 'all') data = data.filter(inv => inv.payment_status === statusFilter);
      
      // Sort by date descending
      data.sort((a, b) => String(b.invoice_date || '').localeCompare(String(a.invoice_date || '')));
