...
import { Calendar, Receipt, FileText, CheckCircle, XCircle, AlertCircle, FileDown, Edit, X, Save, DollarSign, Printer } from 'lucide-react';
...
const AttendanceMarkModal = ({ employee, date, onSave, onCancel }) => {
  ...
  // Handle status change - reset payment to 0 for non-working statuses
  const handleStatusChange = (newStatus) => {
    if (newStatus === 'present') {
      // Full day present - reset to full daily rate
      setFormData({
        ...formData,
        status: newStatus,
        hours_worked: 9,
      ...
      // Half day - half payment
      setFormData({
        ...formData,
        status: newStatus,
        hours_worked: 4.5,
        overtime_hours: 0,
        payment_amount: (employee?.daily_rate || 0) / 2,
      });
    } else {
      // Absent, Sick Leave, Leave, Holiday - No payment
      setFormData({
        ...formData,
        status: newStatus,
        hours_worked: 0,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {(formData.status !== 'present' && formData.status !== 'half_day') && (
        <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg text-sm text-yellow-800 dark:text-yellow-300">
          No payment will be added for {formData.status === 'absent' ? 'Absent' : formData.status === 'sick' ? 'Sick Leave' : formData.status === 'leave' ? 'Leave' : 'Holiday'}
        </div>
      )}
              ...
              <table className="w-full text-sm border-collapse">
                <thead className="bg-gray-100 dark:bg-gray-800">
                  <tr>
                    ...
                    {isEditMode && <th className="px-2 py-1 text-center font-semibold border dark:border-gray-600 dark:text-gray-300">Edit</th>}
                  </tr>
                </thead>
                <tbody>
                  {activeDates.map((date) => {
                    ...
                    return (
                      <tr key={date} className="border dark:border-gray-700">
                        <td className="px-2 py-1 border dark:border-gray-600 dark:text-dark-text">
                          {new Date(date).toLocaleDateString('en-GB')}
                        </td>
                        <td className="px-2 py-1 border dark:border-gray-600 dark:text-dark-text font-semibold">{dayName}</td>
                        ...
                        <td className="px-2 py-1 border dark:border-gray-600 text-right dark:text-dark-text font-semibold">
                          {attendance?.hours_worked || '-'}
                        </td>
                        <td className="px-2 py-1 border dark:border-gray-600 text-right dark:text-dark-text font-semibold">
                          {attendance?.overtime_hours || '-'}
                        </td>
                        <td className="px-2 py-1 border dark:border-gray-600 text-right font-bold dark:text-dark-text">
                  ...
                  {activeDates.map((date, idx) => {
                    ...
                    return (
                      <tr key={date} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-2 py-1 dark:text-dark-text">{dayName}</td>
                        ...
                        <td className="px-2 py-1 text-right dark:text-dark-text">
                          {attendance?.hours_worked || '-'}
                        </td>
                        <td className="px-2 py-1 text-right dark:text-dark-text">
                          {attendance?.overtime_hours || '-'}
